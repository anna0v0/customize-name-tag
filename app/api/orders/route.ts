...
import { NAME_TAG_UNIT_PRICE, ORGANIZER_UNIT_PRICE, ORDER_CURRENCY, orderItemsTotal } from "@/lib/pricing";
...
export async function POST(req:Request){
  ...
  const totalAmount=orderItemsTotal(items);
  ...
  const order={...data,id,createdAt:new Date().toISOString(),status:"Generating" as const,design:items[0].design,items,quantity,unitPrice,totalAmount,currency:ORDER_CURRENCY};
  ...
  return NextResponse.json({orderId:id,unitPrice,totalAmount,currency:ORDER_CURRENCY},{status:201});
}
