...
type ShopifyLine={id:number|string;quantity:number;title?:string;variant_title?:string|null;properties?:Property[];custom_attributes?:Property[]};
...
async function generateModels(order:StoredOrder){
  const items:StoredOrderItem[]=(order.items||[]).map(item=>({...item,modelStatus:"Generating"}));await saveOrder({...order,items});
  const results=await Promise.allSettled(items.map((item,index)=>generate3mf(index===0?order.id:`${order.id}-${index+1}`,designSchema.parse(item.design))));
  results.forEach((result,index)=>{items[index].modelStatus=result.status==="fulfilled"?"Ready":"Failed"});
...
async function paid(payload:ShopifyOrder){
  ...
  for(const line of payload.line_items||[]){
    const designId=property(line,"Design ID");if(!designId)continue;
    const draft=await getDesignDraft(designId);if(!draft||draft.status==="Expired")throw new Error(`Missing design draft ${designId}`);
    items.push({design:draft.design,designId,shopifyLineItemId:String(line.id),quantity:Math.max(1,line.quantity||1),modelStatus:"Queued"});
  }
  if(!items.length)throw new Error("Paid Shopify order has no valid design references.");
