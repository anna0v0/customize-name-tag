...
export default function NameTagDesigner(){
 ...
 function mobileNext(){if(mobileStage<3)selectMobileStage(mobileStage+1);else setStep("details")}
  ...
  <section id="maker" className="maker">
   ...
   {step==="done" ? <div className="success"><div className="success-mark">✓</div><p className="eyebrow">REQUEST RECEIVED</p><h2>We’ll take it from here.</h2><p>Your order reference is <strong>{orderId}</strong>. We’ll review the production model and contact you with a quote before anything is printed.</p><button onClick={()=>setStep("design")}>DESIGN ANOTHER</button></div> : <div className={`studio ${step==="details"?"studio-details":""}`}>
    ...
    <div className="preview-wrap">
      ...
      {step==="design"?<button className="continue order-continue desktop-order-continue" disabled={!design.name} onClick={()=>setStep("details")}>CONTINUE TO ORDER REQUEST <span>→</span></button>:
