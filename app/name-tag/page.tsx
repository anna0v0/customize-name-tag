...
export default function NameTagDesigner(){
 ...
 async function submit(e:React.FormEvent<HTMLFormElement>){e.preventDefault();if(!cart.length)return;setBusy(true);setError("");const f=new FormData(e.currentTarget);const quantity=cart.reduce((total,item)=>total+item.quantity,0);const payload={customerName:f.get("customerName"),email:f.get("email"),phone:f.get("phone"),items:cart.map(item=>({design:item.design,quantity:item.quantity})),notes:f.get("notes"),consent:f.get("consent")=== "on"};const res=await fetch("/api/orders",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(payload)});const data=await res.json();setBusy(false);if(!res.ok){setError(data.error||"We could not submit your order.");return;}setOrderId(data.orderId);setOrderQuantity(quantity);setOrderTotal(data.totalAmount??quantity*NAME_TAG_UNIT_PRICE);setStep("done");}
 return <main>
  <nav><a className="brand" href="/">FORM <i>&</i> FABLE</a><div className="navlinks"><a href="#maker">DESIGN</a><a href="#how">HOW IT WORKS</a></div><a className="bag" href="/">VIEW ALL PRODUCTS</a></nav>
  <header className="hero"><div><h1>Your name,<br/><em>made tangible.</em></h1><p className="intro">Design a one-of-a-kind name tag in minutes.</p></div><img className="hero-art" src="/images/roro-magic.png" alt="Roro Magic"/></header>
  <section id="maker" className="maker">
   ...
   {step==="done" ? <div className="success success-order-page">
    ...
    <div className="preview-wrap">
      ...
      {step==="design"?<button className="continue order-continue desktop-order-continue" disabled={!design.name} onClick={addCurrent}>{editingId?"UPDATE DESIGN":"ADD TO ORDER"} <span>→</span></button>:step==="cart"?<div className="orderform"><div className="formtitle"><h3>Your order</h3><b>{cart.reduce((sum,item)=>sum+item.quantity,0)} TAGS · HK${cart.reduce((sum,item)=>sum+item.quantity,0)*NAME_TAG_UNIT_PRICE}</b></div><div className="order-cart">{cart.map((item,index)=><article key={item.id}><span>{String(index+1).padStart(2,"0")}</span><div className="order-item-preview"><Preview3D design={item.design}/></div><div className="order-item-copy"><strong>{item.design.name}</strong><small>{item.design.font}</small></div><label>QTY<input aria-label={`Quantity for ${item.design.name}`} type="number" min="1" max="100" value={item.quantity} onChange={e=>changeItemQuantity(item.id,Number(e.target.value))}/></label><button type="button" onClick={()=>editItem(item)}>EDIT</button><button type="button" onClick={()=>removeItem(item.id)}>REMOVE</button></article>)}</div><button className="add-another" type="button" onClick={addAnother}>＋ ADD ANOTHER DESIGN</button><button className="continue" type="button" disabled={!cart.length} onClick={continueToDetails}>CONTINUE TO YOUR DETAILS <span>→</span></button></div>:
      <form className="orderform" onSubmit={submit}><div className="formtitle"><button type="button" onClick={()=>setStep("cart")}>← BACK TO YOUR ORDER</button><h3>Your details</h3><b>{cart.reduce((sum,item)=>sum+item.quantity,0)} TAGS · HK${cart.reduce((sum,item)=>sum+item.quantity,0)*NAME_TAG_UNIT_PRICE}</b></div><div className="twocol"><label>FULL NAME<input required name="customerName"/></label><label>EMAIL<input required type="email" name="email"/></label><label>PHONE / WHATSAPP<input required name="phone"/></label></div><label>NOTES (OPTIONAL)<textarea name="notes" maxLength={500}/></label><label className="check"><input required name="consent" type="checkbox"/> I agree that my artwork will be stored for order fulfilment.</label><button className="continue" disabled={busy||!cart.length}>{busy?"PLACING ORDER…":"PLACE ORDER"}<span>→</span></button></form>}
    </div>
