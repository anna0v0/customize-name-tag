...
const OrganizerPreview3D=dynamic(()=>import("@/components/OrganizerPreview3D"),{ssr:false,loading:()=> <div className="loading">Building preview…</div>});

function OrderDesignPreview({design}:{design:PublicOrderSummary["designs"][number]["design"]}){return "productType" in design?<OrganizerPreview3D design={design}/>:<Preview3D design={design}/>}
...
function StatusCard({order}:{order:PublicOrderSummary}){
  ...
  return <article className="lookup-card">
    <button className="lookup-card-toggle" type="button" aria-expanded={expanded} aria-controls={`order-${order.orderId}`} onClick={()=>setExpanded(value=>!value)}><div><small>{new Intl.DateTimeFormat("en-HK",{dateStyle:"medium"}).format(new Date(order.createdAt))}</small><h2>{order.orderId}</h2></div><span className={`lookup-status ${order.status.toLowerCase().replaceAll(" ","-")}`}>{order.status}</span><i aria-hidden="true">{expanded?"−":"＋"}</i></button>
    {expanded&&<div className="lookup-card-content" id={`order-${order.orderId}`}>
      <div className="lookup-meta"><div><small>ORDERED</small><b>{new Intl.DateTimeFormat("en-HK",{dateStyle:"medium"}).format(new Date(order.createdAt))}</b></div><div><small>QUANTITY</small><b>{order.quantity} {order.quantity===1?"ITEM":"ITEMS"}</b></div><div><small>SHIPPING</small><b>{order.shippingLabel??"Not recorded"}{order.shippingFee!=null?` · HK$${order.shippingFee}`:""}</b></div><div><small>TOTAL</small><b>{order.currency} ${order.totalAmount}</b></div></div>
      <div className="lookup-designs"><small>DESIGNS</small>{order.designs.map((design,index)=><div key={`${design.name}-${index}`}><span>{String(index+1).padStart(2,"0")}</span><div className="lookup-design-preview"><OrderDesignPreview design={design.design}/></div><b>{design.name}</b><small>QTY {design.quantity}</small></div>)}</div>
...
export default function OrderStatusPage(){
  ...
  return <main className="order-lookup-page">
    <nav><a className="brand" href="/">THE <span className="brand-accent">ODDMENT</span> CLUB</a><div className="navlinks"><a href="/#products">PRODUCTS</a><a className="active" href="/order-status">ORDER STATUS</a></div></nav>
    <section className="lookup-hero"><h1>TRACK YOUR ORDER</h1><p>Enter your full order reference or the email address used at checkout.</p>
