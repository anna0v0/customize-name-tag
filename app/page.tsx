...
export default function Home(){
 ...
 const [design,setDesign]=useState<DesignConfig>({name:"MILO",font:"Block",baseColor:"#f4f0e7",topColor:"#1e1f22",icon:"flower",templateVersion:"1"});
 ...
 async function upload(file?:File){if(!file)return;setError("");if(file.size>5*1024*1024||!['image/png','image/svg+xml'].includes(file.type)){setError("Choose a PNG or SVG file under 5 MB.");return;}const fd=new FormData();fd.append("file",file);const res=await fetch("/api/uploads/icon",{method:"POST",body:fd});const data=await res.json();if(!res.ok){setError(data.error);return;}setUploadName(file.name);patch("icon","upload");patch("iconDataUrl",data.dataUrl);}
  ...
  <section id="maker" className="maker">
   ...
   {step==="done" ? <div className="success"><div className="success-mark">✓</div><p className="eyebrow">REQUEST RECEIVED</p><h2>We’ll take it from here.</h2><p>Your order reference is <strong>{orderId}</strong>. We’ll review the production model and contact you with a quote before anything is printed.</p><button onClick={()=>setStep("design")}>DESIGN ANOTHER</button></div> : <div className="studio">
    <aside className="controls">
     ...
     <div className="field"><span><b>5</b> ICON</span><div className="icons">{([['flower','✿'],['star','★'],['heart','♥']] as const).map(([v,s])=><button className={design.icon===v?"selected":""} key={v} onClick={()=>patch("icon",v)}>{s}</button>)}<label className={design.icon==='upload'?"selected upload":"upload"}>＋<input type="file" accept=".png,.svg,image/png,image/svg+xml" onChange={e=>upload(e.target.files?.[0])}/></label></div>{uploadName&&<small>Using {uploadName}</small>}<small>Solid silhouette PNG or SVG · max 5 MB</small></div>
    </aside>
