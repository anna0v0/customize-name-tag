...
export default function Home(){
 ...
 async function upload(file?:File,avatarSelection?:AvatarSelection){if(!file)return false;setError("");if(file.size>1024*1024||file.type!=="image/svg+xml"){setError("Choose a vector SVG file under 1 MB.");return false;}const fd=new FormData();fd.append("file",file);const res=await fetch("/api/uploads/icon",{method:"POST",body:fd});const data=await res.json();if(!res.ok){setError(data.error);return false;}setUploadName(file.name);setDesign(d=>({...d,icon:"upload",iconDataUrl:data.dataUrl,iconAssetId:data.assetId,iconContours:data.contours,avatarSelection}));return true;}
 async function applyAvatar(selection:AvatarSelection,svg:string){const accepted=await upload(new File([svg],"custom-avatar.svg",{type:"image/svg+xml"}),selection);if(accepted)setAvatarOpen(false)}
 function removeAvatar(){setUploadName("");setDesign(d=>({...d,icon:"flower",iconDataUrl:undefined,iconAssetId:undefined,iconContours:undefined,avatarSelection:undefined}))}
  ...
  <section id="maker" className="maker">
   ...
   {step==="done" ? <div className="success"><div className="success-mark">✓</div><p className="eyebrow">REQUEST RECEIVED</p><h2>We’ll take it from here.</h2><p>Your order reference is <strong>{orderId}</strong>. We’ll review the production model and contact you with a quote before anything is printed.</p><button onClick={()=>setStep("design")}>DESIGN ANOTHER</button></div> : <div className="studio">
    <aside className="controls">
     ...
     <div className="field"><span><b>5</b> ICON</span><div className="icons">{([['flower','✿'],['star','★'],['heart','♥']] as const).map(([v,s])=><button className={design.icon===v?"selected":""} key={v} onClick={()=>patch("icon",v)}>{s}</button>)}<label className={design.icon==='upload'&&!design.avatarSelection?"selected upload":"upload"}>＋<input type="file" accept=".svg,image/svg+xml" onChange={e=>upload(e.target.files?.[0])}/></label></div><button className="avatar-create" onClick={()=>setAvatarOpen(true)}>{design.avatarSelection?"EDIT AVATAR":"CREATE AVATAR"}<span>→</span></button>{design.avatarSelection&&<button className="avatar-remove" onClick={removeAvatar}>REMOVE AVATAR</button>}{uploadName&&<small>Using {uploadName}</small>}<small>Dark SVG fills or outlined strokes · max 1 MB</small><div className="icon-size"><label htmlFor="icon-size"><span>ICON SIZE</span><strong>{Math.round((design.iconScale??1)*100)}%</strong></label><input id="icon-size" type="range" min="0.7" max="1.5" step="0.05" value={design.iconScale??1} onChange={e=>patch("iconScale",Number(e.target.value))}/></div></div>
    </aside>
    <div className="preview-wrap">
      ...
      <div className="specbar"><div><small>PRINT SCALE</small><strong>{Math.round(layout.scale*100)}%</strong></div><div><small>OUTLINE</small><strong>3 mm</strong></div><div><small>MIN. FEATURE</small><strong>{layout.minFeature.toFixed(2)} mm</strong></div><div className={layout.minFeature>=.8?"pass":"warn"}><small>PRINT CHECK</small><strong>{layout.minFeature>=.8?"✓ READY":"! REVIEW"}</strong></div></div>
