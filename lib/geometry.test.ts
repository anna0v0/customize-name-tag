...
describe("preset icons",()=>{it("generates every supplied SVG and keeps the flower centre hole",()=>{expect(PRESET_ICON_CONTOURS.flower.some(contour=>contour.hole)).toBe(true);for(const icon of ["flower","cat","paw","cloud","file","heart","star","thunder","bow-tie","crown"] as const){expect(PRESET_ICON_CONTOURS[icon].length).toBeGreaterThan(0);const g=createTagGeometry({name:"Milo",font:"Gochi Hand",baseColor:"#ffffff",topColor:"#000000",icon,templateVersion:"1"});expect(g.face.getAttribute("position").count).toBeGreaterThan(0);g.base.dispose();g.face.dispose();g.ring.dispose()}})});
