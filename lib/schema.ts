...
export const designSchema = z.object({
  ...
  icon: z.enum(["star","heart","flower","upload"]), iconScale: z.number().min(.7).max(1.5).optional(), iconDataUrl: z.string().optional(), iconAssetId: z.string().regex(/^[a-f0-9-]{36}$/).optional(),
  iconContours: z.array(z.object({group:z.number().int().min(0).max(11),hole:z.boolean(),points:z.array(z.object({x:z.number().min(-1.1).max(1.1),y:z.number().min(-1.1).max(1.1)})).min(3).max(2000)})).max(48).optional(), templateVersion: z.literal("1")
});
