...
import darumadropOne from "./fonts/darumadrop-one.typeface.json";
import caveatBrush from "./fonts/caveat-brush.typeface.json";
import cherryBombOne from "./fonts/cherry-bomb-one.typeface.json";
import eastSeaDokdo from "./fonts/east-sea-dokdo.typeface.json";
import sourGummyBold from "./fonts/sour-gummy-bold.typeface.json";
import bungee from "./fonts/bungee.typeface.json";
import { DesignConfig, iconPlacement, layoutFor, SPECS } from "./config";
import { PRESET_ICON_CONTOURS } from "./preset-icons";
...
function contourShapes(contours:NonNullable<DesignConfig["iconContours"]>,cx:number,cy:number,size:number){const groups=new Map<number,Shape>();for(const contour of contours){const points=contour.points.map(p=>({x:cx+p.x*size,y:cy+p.y*size}));if(contour.hole){groups.get(contour.group)?.holes.push(polygon(points))}else groups.set(contour.group,polygon(points))}return[...groups.values()]}
function uploadedShapes(design:DesignConfig,cx:number,cy:number,size:number){return contourShapes(design.iconContours||[],cx,cy,size)}
function iconShapes(design:DesignConfig,cx:number,cy:number,size:number):Shape[]{const icon=design.icon;if(icon in PRESET_ICON_CONTOURS)return contourShapes(PRESET_ICON_CONTOURS[icon as keyof typeof PRESET_ICON_CONTOURS],cx,cy,size);if(icon==="star"){const p:Point[]=[];for(let i=0;i<10;i++){const a=-Math.PI/2+i*Math.PI/5,r=i%2?size*.45:size;p.push({x:cx+Math.cos(a)*r,y:cy+Math.sin(a)*r})}return[polygon(p)]}if(icon==="heart"){const s=new Shape();s.moveTo(cx,cy-size*.75);s.bezierCurveTo(cx-size*1.2,cy-size*.05,cx-size*.85,cy+size*.85,cx,cy+size*.35);s.bezierCurveTo(cx+size*.85,cy+size*.85,cx+size*1.2,cy-size*.05,cx,cy-size*.75);return[s]}const uploaded=uploadedShapes(design,cx,cy,size);return uploaded.length?uploaded:[circle(cx,cy,size*.82)]}
function transformedText(design:DesignConfig,width:number,x0:number){const size=SPECS.targetTextHeight;const font=selectedFont(design.font);const name=design.name||"NAME";const raw=font.generateShapes(name,size);const samples=raw.flatMap(s=>sampled(s).outer);const minX=Math.min(...samples.map(p=>p.x)),maxX=Math.max(...samples.map(p=>p.x));const available=width-x0-2;const factor=Math.min(1,available/(maxX-minX));const alignmentName=name.replace(/[gjpqy]/g,"")||name.replace(/[gjpqy]/g,"x");const alignmentSamples=font.generateShapes(alignmentName,size).flatMap(s=>sampled(s).outer);const alignmentMinY=Math.min(...alignmentSamples.map(p=>p.y)),alignmentMaxY=Math.max(...alignmentSamples.map(p=>p.y));const baseline=(23-(alignmentMaxY-alignmentMinY)*factor)/2-alignmentMinY*factor;return raw.map(s=>{const p=sampled(s);const outer=p.outer.map(v=>({x:x0+(v.x-minX)*factor,y:baseline+v.y*factor}));const out=polygon(outer);out.holes=p.holes.map(h=>polygon(h.map(v=>({x:x0+(v.x-minX)*factor,y:baseline+v.y*factor}))));return out})}
...
export function createTagGeometry(design:DesignConfig){const placement=iconPlacement(design.iconScale);const layout=layoutFor(design.name,placement.scale);const text=transformedText(design,layout.width,placement.textX);const icons=iconShapes(design,placement.centerX,11.5,SPECS.iconSize*placement.scale);const faces=[...icons,...text];const baseShapes=offsetSilhouette(faces,SPECS.outline);const base=new ExtrudeGeometry(baseShapes,{depth:SPECS.baseDepth,bevelEnabled:false,curveSegments:8});const overlap=.05;const face=new ExtrudeGeometry(faces,{depth:SPECS.faceDepth+overlap,bevelEnabled:false,curveSegments:8});face.translate(0,0,SPECS.baseDepth-overlap);const ringShape=circle(SPECS.ringCenterX,11.5,SPECS.holeDiameter/2+SPECS.ringWall);ringShape.holes.push(circle(SPECS.ringCenterX,11.5,SPECS.holeDiameter/2));const ring=new ExtrudeGeometry(ringShape,{depth:SPECS.baseDepth,bevelEnabled:false,curveSegments:24});return{base,face,ring,layout}}
