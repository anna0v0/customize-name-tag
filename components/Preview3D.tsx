...
export default function Preview3D({design}:{design:DesignConfig}){return <div className="canvas"><Canvas camera={{position:[0,5.5,8.5],fov:35}} shadows gl={{antialias:true,alpha:false}}><color attach="background" args={["#d9d5cb"]}/><ambientLight intensity={2}/><directionalLight position={[4,7,6]} intensity={3} castShadow/><Model design={design}/><ContactShadows position={[0,-2.15,0]} opacity={.3} scale={13} blur={2.3}/><OrbitControls enablePan={false} minDistance={6} maxDistance={14}/></Canvas></div>}
